-- FUTURE AUTOMOTIVE - Hostinger Ultimate Fix
-- Alternative approach for Hostinger hosting

-- Step 1: Try to disable foreign key checks (might work in Hostinger)
SET FOREIGN_KEY_CHECKS = 0;

-- Step 2: Try to drop tables one by one (execute each separately)
...
-- Step 3: Alternative approach - use TRUNCATE if DROP doesn't work
TRUNCATE TABLE notifications;
TRUNCATE TABLE breakdown_audit_log;
TRUNCATE TABLE breakdown_time_logs;
TRUNCATE TABLE breakdown_work_items;
TRUNCATE TABLE breakdown_assignments;
TRUNCATE TABLE breakdown_reports;
TRUNCATE TABLE articles_catalogue;
TRUNCATE TABLE drivers;
TRUNCATE TABLE users;

-- Step 4: Drop and recreate with different approach
DROP TABLE IF EXISTS notifications;
DROP TABLE IF EXISTS breakdown_audit_log;
DROP TABLE IF EXISTS breakdown_time_logs;
DROP TABLE IF EXISTS breakdown_work_items;
...
-- Step 6: Create users table (without foreign keys first)
CREATE TABLE users (
...
-- Step 9: Create articles_catalogue table
...
-- Step 10: Create breakdown_reports table (without foreign keys first)
...
-- Step 11: Add foreign keys to breakdown_reports
ALTER TABLE breakdown_reports ADD FOREIGN KEY (driver_id) REFERENCES drivers(id) ON DELETE SET NULL;
ALTER TABLE breakdown_reports ADD FOREIGN KEY (bus_id) REFERENCES buses(id) ON DELETE SET NULL;
ALTER TABLE breakdown_reports ADD FOREIGN KEY (created_by_user_id) REFERENCES users(id) ON DELETE SET NULL;
ALTER TABLE breakdown_reports ADD FOREIGN KEY (updated_by_user_id) REFERENCES users(id) ON DELETE SET NULL;

-- Step 12: Create breakdown_assignments table (without foreign keys first)
...
-- Step 13: Add foreign keys to breakdown_assignments
ALTER TABLE breakdown_assignments ADD FOREIGN KEY (report_id) REFERENCES breakdown_reports(id) ON DELETE CASCADE;
ALTER TABLE breakdown_assignments ADD FOREIGN KEY (assigned_to_user_id) REFERENCES users(id) ON DELETE RESTRICT;
ALTER TABLE breakdown_assignments ADD FOREIGN KEY (assigned_by_user_id) REFERENCES users(id) ON DELETE RESTRICT;

-- Step 14: Create breakdown_work_items table (without foreign keys first)
...
-- Step 15: Add foreign keys to breakdown_work_items
ALTER TABLE breakdown_work_items ADD FOREIGN KEY (breakdown_id) REFERENCES breakdown_reports(id) ON DELETE CASCADE;
ALTER TABLE breakdown_work_items ADD FOREIGN KEY (assignment_id) REFERENCES breakdown_assignments(id) ON DELETE CASCADE;
ALTER TABLE breakdown_work_items ADD FOREIGN KEY (article_id) REFERENCES articles_catalogue(id) ON DELETE RESTRICT;
ALTER TABLE breakdown_work_items ADD FOREIGN KEY (added_by_user_id) REFERENCES users(id) ON DELETE RESTRICT;

-- Step 16: Create breakdown_time_logs table (without foreign keys first)
...
-- Step 17: Add foreign keys to breakdown_time_logs
ALTER TABLE breakdown_time_logs ADD FOREIGN KEY (breakdown_id) REFERENCES breakdown_reports(id) ON DELETE CASCADE;
ALTER TABLE breakdown_time_logs ADD FOREIGN KEY (assignment_id) REFERENCES breakdown_assignments(id) ON DELETE CASCADE;
ALTER TABLE breakdown_time_logs ADD FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE RESTRICT;

-- Step 18: Create breakdown_audit_log table
...
-- Step 19: Create notifications table (without foreign keys first)
...
-- Success message
SELECT '✅ Hostinger ultimate database reset completed successfully!' as message;
